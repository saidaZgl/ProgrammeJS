...
function afficherMenu() {
  ...
  textMenu += "2 - afficher le joueur le plus âgé\n";
  textMenu += "3 - calculer la moyenne d'âge des joueurs\n";
  textMenu += "4 - ajouter un joueur\n";
  textMenu += "9 - quitter le programme\n";
...
function afficherLeJoueurLePlusAge() {
  ...
  for (var i = 1; i < lesJoueurs.length; i++) {
    ...
    console.log("Le joueur le plus âgé est :" + joueurLePlusAge.nom);
  }
  afficherLigneSeparation(SEPARATEUR);
...
function afficherDifferenceAgeJoueurs(premierJoueur, deuxiemeJoueur) {
  ...
  console.log(
    "La difference d'age entre les deux joueurs est de " +
      differenceAgeJoueurs +
      " ans"
  );
  afficherLigneSeparation(SEPARATEUR);
}
